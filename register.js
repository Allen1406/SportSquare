document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', function(event) {
        ...
        if (!form.firstname.value) {
            ...
            document.getElementById('firstname-input').classList.add('error-outline');
            hasErrors = true;
        }

        if (!form.lastname.value) {
            errors.lastname.textContent = 'Please enter your last name.';
            document.getElementById('lastname-input').classList.add('error-outline');
            hasErrors = true;
        }

        if (!form.collegename.value) {
            errors.collegename.textContent = 'Please enter your college name.';
            document.getElementById('collegename-input').classList.add('error-outline');
            hasErrors = true;
        }
        //validate college year
        if (collegeYearInput.value.length !== 1 || collegeYearInput.value < 0 || collegeYearInput.value > 9) {
            errors.collegeYear.textContent = 'Please enter a valid single-digit college year.';
            collegeYearInput.classList.add('error-outline');
            hasErrors = true;
        }
        ...
        if (!dobValue.match(datePattern) || !isValidDate(dobValue)) {
            errors.dob.textContent = 'Please enter a valid date in DD/MM/YYYY format.';
            dobInput.classList.add('error-outline');
            hasErrors = true;
        }
        ...
        if (!form.email.value) {
            ...
            document.getElementById('email-input').classList.add('error-outline');
            hasErrors = true;
        }
        //validate password
        if (password.value.length < 8 || password.value.length > 16) {
            errors.password.textContent = 'Password must be between 8 and 16 characters.';
            password.classList.add('error-outline');
            hasErrors = true;
        }

        if (password.value !== repeatPassword.value) {
            errors.repeatPassword.textContent = 'Passwords do not match.';
            repeatPassword.classList.add('error-outline');
            hasErrors = true;
        }
